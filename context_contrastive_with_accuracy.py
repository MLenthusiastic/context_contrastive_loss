...
def context_aggregated_pair_selection( batch_img_objects, batch_classes):
    batch_z_vectors = []
    ...
    return context_z_pair_img_1, context_z_pair_img_2, targets, class_lables, pair_img1, pair_img2
...
train_full_dataset = ContextDavis(train_davis_json, full_train_davis_memmap)
...
train_davis_dataloader = torch.utils.data.DataLoader(train_full_dataset, batch_size=args.batch_size,
                                                     sampler=train_sampler,collate_fn=collate_for_same_order )
eval_davis_dataloader = torch.utils.data.DataLoader(train_full_dataset, batch_size=args.batch_size,
                                                    sampler=valid_sampler,collate_fn=collate_for_same_order)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    for dataloader in [train_davis_dataloader, eval_davis_dataloader, test_davis_dataloader]:
        ...
        for batch in dataloader:
            ...
            context_z_pair_img_1, context_z_pair_img_2, targets, class_lables, pair_img1, pair_img2 = \
                context_aggregated_pair_selection(batch_img_objects, batch_classes)

            if dataloader == train_davis_dataloader:
                context_model.train()
                torch.set_grad_enabled(True)
                stage = 'train'
            elif dataloader == eval_davis_dataloader:
                context_model.eval()
                torch.set_grad_enabled(False)
                stage = 'eval'
            else:
    ...
    for dataloader in [train_davis_dataloader, test_davis_dataloader]:
        ...
        for batch in dataloader:
            ...
            class_1 = []
            class_2 = []
            for class_pair in class_lables:
                class_1.append(class_pair[0])
                class_2.append(class_pair[1])
            ...
            center_list_img1 = {label: context_aggregated_z1[np.where(class_1_np == label)[0]] for label in labels_set}
            center_list_img2 = {label: context_aggregated_z2[np.where(class_2_np == label)[0]] for label in labels_set}
            ...
            batch_out1_closet = []
            batch_out2_closet = []
            for o1, o2 in zip(context_aggregated_z1, context_aggregated_z2):
            ...
            for i in range(len(batch_out1_closet)):
                if torch.equal(torch.Tensor(out1_encoded[i]), torch.Tensor(out2_encoded[i])) and targets[i] == 0:
                    n_correct += 1
                elif ~torch.equal(torch.Tensor(out1_encoded[i]), torch.Tensor(out2_encoded[i])) and targets[i] == 1:
                    n_correct += 1
